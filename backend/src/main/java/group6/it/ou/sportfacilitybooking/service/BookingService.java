...
public class BookingService {
    ...
    public BookingDTO createBooking(BookingCreateRequest request, Long customerId) {
        ...
        // ✅ NEW: Validate payment time - current datetime must be before booking date +
        // start time
        ...
        if (!now.isBefore(bookingDateTime)) {
            throw new RuntimeException(
                    "Thời gian hiện tại đã vượt quá thời gian đặt sân. Vui lòng chọn thời gian khác");
        }
        ...
        booking.setDepositAmount(
                booking.getTotalPrice().multiply(timeSlot.getDepositRate()).divide(new BigDecimal(100)));
    ...
    public void autoConfirmPendingBookingsForNextDay() {
        ...
        List<Booking> bookingsToConfirm = bookingRepository
                .findByStatusAndCreatedAtBefore(BookingStatus.PENDING_CONFIRM, startOfToday);
    ...
    private void createNotification(User user, NotificationType type, String title, String message, Long refId,
            String refType) {
