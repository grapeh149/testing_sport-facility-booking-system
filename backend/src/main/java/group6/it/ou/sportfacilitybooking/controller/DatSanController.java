...
import group6.it.ou.sportfacilitybooking.service.DatSanService;
...
public class DatSanController {
    ...
    @PostMapping
    public ChiTietDatSanDTO createBooking(
    ...
    // Tạm truyền maKH qua query param vì chưa có JWT
    @GetMapping("/ca-nhan")
    public List<DatSanDTO> getMyBookings(@RequestParam Integer maKH) {
        return datSanService.getByMaKH(maKH);
    }

    // API-22: GET /api/dat-san/chi-tiet/{id}
    @GetMapping("/chi-tiet/{id}")
    public ChiTietDatSanDTO getBookingDetail(@PathVariable("id") Integer maDatSan) {
        return datSanService.getByMaDatSan(maDatSan);
    }

    // API-23: PATCH /api/dat-san/{id}/huy
    @PatchMapping("/{id}/huy")
    public TrangThaiDatSanRequest cancelBooking(@PathVariable("id") Integer maDatSan) {
        return datSanService.cancelByCustomer(maDatSan);
    }

    // API-24: PATCH /api/dat-san/{id}/xac-nhan
    @PatchMapping("/{id}/xac-nhan")
    public TrangThaiDatSanRequest confirmBooking(@PathVariable("id") Integer maDatSan) {
        return datSanService.confirmBooking(maDatSan);
    }

    // API-25: PATCH /api/dat-san/{id}/tu-choi
    @PatchMapping("/{id}/tu-choi")
    public TrangThaiDatSanRequest rejectBooking(@PathVariable("id") Integer maDatSan) {
        return datSanService.rejectBooking(maDatSan);
    }

    // API-26: POST /api/dat-san/{id}/checkin
    @PatchMapping("/{id}/checkin")
    public TrangThaiDatSanRequest checkinBooking(@PathVariable("id") Integer maDatSan) {
    ...
    public List<DatSanDTO> getBookingsByFacility(@PathVariable Integer facilityId) {
